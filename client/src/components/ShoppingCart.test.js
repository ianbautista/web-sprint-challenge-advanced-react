...
test("testtest if all mock plants renders in shopping cart ", () => {
	const { getByText } = render(<ShoppingCart cart={plants} />);
	const plantSelected1 = screen.getByText(/Snake Plant/i);
	const plantSelected2 = screen.getByText(/String of Dolphins/i);
	const plantSelected3 = screen.getByText(/Peperomia Rosso/i);

	expect(plantSelected1).toBeInTheDocument();
	expect(plantSelected2).toBeInTheDocument();
	expect(plantSelected3).toBeInTheDocument();
