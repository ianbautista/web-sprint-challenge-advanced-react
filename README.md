...
`We first start by declaring our class component by extending the React.Component parent class. Like, class SampleComponent extends React.Component {}. Use the constructor function to set up some state. because we’re calling extends, we also need to call super(); otherwise we won’t have access the this. Then we set state by writing this.state, in class components we only write our states like this. Then we need to render some sort of UI to the DOM. We do this by calling the life-cycle method render().`

2. Describe the different phases of the component lifecycle.
   `The Mounting Phase invokes render method and calls componentDidMount if needed. The Updating Phase executes setState and forces another render after each update. The Unmounting Phase removes the component from the screen and calls componentWillUnmount if needed.`

3. Demonstrate an understanding of class component lifecycle methods.
   `componentDidMount - it fires after the component initially mounts to the DOM. The DOM is then accessible and can be modified. It is also a good time to perform network requests and other asynchronous tasks, so that the application's state will be updated after the request resolves.`

`componentDidUpdate - it will fire immediately after the DOM is updated, but not on the initial render. This allows us to compare the previous and current state or props between updates. It's a good time to perform asynchronous tasks if they are necessary, and also to perform any DOM operations.`

`componentWillUnmount - it will fire before a component is unmounted. It's useful for performing any cleanup operations, like canceling network requests, subscriptions, etc.`

4. Define stateful logic.
   `Stateful logic is any code that uses state. Some custom hooks can contain or considered stateful logic. While their state is completely independent, they are reusable across your app.`

5. Describe how to test a React component with React Testing Library.
   `Plan your test first. Write a blueprint or guide of what you want to test. Setup your test file. import the file your are testing and import RTL. Do an initial test by invoking render on the component you're testing first, then write out your test as planned. Keep in mind that you test for functionality that the user interacts with. So first step is querying for elements then firing eventsto test them and finally asserting your tests.`
